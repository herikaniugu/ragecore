function Setup() {
    ...
    if (Engine.VERSION > "1.0") {
        ...
        // EXTEND
        App.draw.wait = function(value) {
            var frame = this.frame * (this.rate || 1); if (frame == 1 || (frame / value) % 1 == 0) return true; return false;
        };
        // BRIDGE
        Script().preload(Resources(Path(["Source", App.VERSION, "Bridge"]), "bridge").files(["Load", "Value", "About", "Menu", "Main", "Score"])).response(function(total, count) {
            App.draw.rate = Math.min(2, Math.abs(App.draw.interpolation) > 2 ? 2 : Math.round(App.draw.interpolation)); App.draw.progress = count / total; if (count == 1) App.draw.open(App.source.load);
        });
        // AD
